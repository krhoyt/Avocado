...
export default {
  ...
  actions: {
    CREATE_RELATIONSHIP: async function( context ) {
      let result = await Relationship.create( context.rootGetters.TOKEN, {
      ...
      context.rootGetters['RELATIONSHIPS'].push( result );
      context.rootGetters['RELATIONSHIPS'].sort( ( a, b ) => {
        if( a.name > b.name ) return 1;
        if( a.name < b.name ) return -1;
        return 0
      } );      

      for( let c = 0; c < context.rootGetters['RELATIONSHIPS'].length; c++ ) {
        if( context.rootGetters['RELATIONSHIPS'][c].id === result.id ) {
          context.commit( 'SET_ORIGINAL', c );
          break;
        }
      }      
    },
    REMOVE_RELATIONSHIP: function( context, id ) {
      Relationship.remove( context.rootGetters.TOKEN, id );
    },
    ...
    UPDATE_RELATIONSHIP: async function( context ) {
      let result = await Relationship.update( context.rootGetters.TOKEN, {
        id: context.getters.ID,
        name: context.getters.NAME,
        color_id: context.rootGetters.COLORS[context.getters.COLOR].id
      } );

      context.rootGetters.RELATIONSHIPS[context.getters.ORIGINAL].name = result.name;
      context.rootGetters.RELATIONSHIPS[context.getters.ORIGINAL].color_id = result.color_id;
