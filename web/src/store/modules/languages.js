...
export default {
  ...
  actions: {
    CREATE_LANGUAGE: async function( context ) {
      let result = await Language.create( context.rootGetters.TOKEN, {
      ...
      context.rootGetters['LANGUAGES'].push( result );
      context.rootGetters['LANGUAGES'].sort( ( a, b ) => {
        if( a.name > b.name ) return 1;
        if( a.name < b.name ) return -1;
        return 0
      } );      

      for( let c = 0; c < context.rootGetters['LANGUAGES'].length; c++ ) {
        if( context.rootGetters['LANGUAGES'][c].id === result.id ) {
          context.commit( 'SET_ORIGINAL', c );
          break;
        }
      }      
    },
    REMOVE_LANGUAGE: function( context, id ) {
      Language.remove( context.rootGetters.TOKEN, id );
    },
    ...
    UPDATE_LANGUAGE: async function( context ) {
      let result = await Language.update( context.rootGetters.TOKEN, {
        id: context.getters.ID,
        name: context.getters.NAME,
        color_id: context.rootGetters.COLORS[context.getters.COLOR].id
      } );

      context.rootGetters.LANGUAGES[context.getters.ORIGINAL].name = result.name;
      context.rootGetters.LANGUAGES[context.getters.ORIGINAL].color_id = result.color_id;
