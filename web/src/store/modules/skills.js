...
export default {
  ...
  actions: {
    CREATE_SKILL: async function( context ) {
      let result = await Skill.create( context.rootGetters.TOKEN, {
      ...
      context.rootGetters['SKILLS'].push( result );
      context.rootGetters['SKILLS'].sort( ( a, b ) => {
        if( a.name > b.name ) return 1;
        if( a.name < b.name ) return -1;
        return 0
      } );      

      for( let c = 0; c < context.rootGetters['SKILLS'].length; c++ ) {
        if( context.rootGetters['SKILLS'][c].id === result.id ) {
          context.commit( 'SET_ORIGINAL', c );
          break;
        }
      }      
    },
    REMOVE_SKILL: function( context, id ) {
      Skill.remove( context.rootGetters.TOKEN, id );
    },
    ...
    UPDATE_SKILL: async function( context ) {
      let result = await Skill.update( context.rootGetters.TOKEN, {
        id: context.getters.ID,
        name: context.getters.NAME,
        color_id: context.rootGetters.COLORS[context.getters.COLOR].id
      } );

      context.rootGetters.SKILLS[context.getters.ORIGINAL].name = result.name;
      context.rootGetters.SKILLS[context.getters.ORIGINAL].color_id = result.color_id;
