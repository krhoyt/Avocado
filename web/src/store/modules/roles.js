...
export default {
  ...
  actions: {
    CREATE_ROLE: async function( context ) {
      let result = await Role.create( context.rootGetters.TOKEN, {
      ...
      context.rootGetters['ROLES'].push( result );
      context.rootGetters['ROLES'].sort( ( a, b ) => {
        if( a.name > b.name ) return 1;
        if( a.name < b.name ) return -1;
        return 0
      } );      

      for( let c = 0; c < context.rootGetters['ROLES'].length; c++ ) {
        if( context.rootGetters['ROLES'][c].id === result.id ) {
          context.commit( 'SET_ORIGINAL', c );
          break;
        }
      }      
    },
    REMOVE_ROLE: function( context, id ) {
      Role.remove( context.rootGetters.TOKEN, id );
    },
    ...
    UPDATE_ROLE: async function( context ) {
      let result = await Role.update( context.rootGetters.TOKEN, {
        id: context.getters.ID,
        name: context.getters.NAME,
        color_id: context.rootGetters.COLORS[context.getters.COLOR].id
      } );

      context.rootGetters.ROLES[context.getters.ORIGINAL].name = result.name;
      context.rootGetters.ROLES[context.getters.ORIGINAL].color_id = result.color_id;
