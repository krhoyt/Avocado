...
export default {
  ...
  actions: {
    CREATE_SITUATION: async function( context ) {
      let result = await Situation.create( context.rootGetters.TOKEN, {
      ...
      context.rootGetters['SITUATIONS'].push( result );
      context.rootGetters['SITUATIONS'].sort( ( a, b ) => {
        if( a.name > b.name ) return 1;
        if( a.name < b.name ) return -1;
        return 0
      } );      

      for( let c = 0; c < context.rootGetters['SITUATIONS'].length; c++ ) {
        if( context.rootGetters['SITUATIONS'][c].id === result.id ) {
          context.commit( 'SET_ORIGINAL', c );
          break;
        }
      }      
    },
    REMOVE_SITUATION: function( context, id ) {
      Situation.remove( context.rootGetters.TOKEN, id );
    },
    ...
    UPDATE_SITUATION: async function( context ) {
      let result = await Situation.update( context.rootGetters.TOKEN, {
        id: context.getters.ID,
        name: context.getters.NAME,
        color_id: context.rootGetters.COLORS[context.getters.COLOR].id
      } );

      context.rootGetters.SITUATIONS[context.getters.ORIGINAL].name = result.name;
      context.rootGetters.SITUATIONS[context.getters.ORIGINAL].color_id = result.color_id;
