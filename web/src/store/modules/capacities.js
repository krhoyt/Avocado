...
export default {
  ...
  actions: {
    CREATE_CAPACITY: async function( context ) {
      ...
      context.rootGetters['CAPACITIES'].push( result );
      context.rootGetters['CAPACITIES'].sort( ( a, b ) => {
        if( a.name > b.name ) return 1;
        if( a.name < b.name ) return -1;
        return 0
      } );      

      for( let c = 0; c < context.rootGetters['CAPACITIES'].length; c++ ) {
        if( context.rootGetters['CAPACITIES'][c].id === result.id ) {
    ...
    UPDATE_CAPACITY: async function( context ) {
      ...
      context.rootGetters.CAPACITIES[context.getters.ORIGINAL].name = result.name;
      context.rootGetters.CAPACITIES[context.getters.ORIGINAL].color_id = result.color_id;
      context.rootGetters.CAPACITIES[context.getters.ORIGINAL].weight = result.weight;
